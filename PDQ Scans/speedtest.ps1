...
param (
    ...
    [String]$DownloadUri = 'https://install.speedtest.net/app/cli/ookla-speedtest-1.1.1-win64.zip',
...
if ( -not (Test-Path -Path $DownloadLocation) ) {
    ...
    $null = New-Item -Path $DownloadLocation -ItemType "Directory" -Force -ErrorAction Stop
...
if ( -not (Test-Path -Path "speedtest.exe") ) {
    ...
    Invoke-RestMethod -Uri $DownloadUri -OutFile "speedtest.zip" -ErrorAction Stop
    Write-Verbose -Message "Download finished."

    Expand-Archive -Path "speedtest.zip" -DestinationPath . -Force -ErrorAction Stop
    Write-Verbose -Message "Unzipped speedtest.zip"
    $null = Remove-Item -Path "speedtest.zip" -ErrorAction Stop

}

#Running speedtest.exe, accepting license agreements, and converting the results
Write-Verbose -Message "Running speedtest.exe"

$SpeedtestResultsJson = .\speedtest.exe --format=json --accept-license --accept-gdpr 2>$null
...
# Save the results.
$SpeedtestResultsJson | Out-File -FilePath "PreviousResults.json" -Force -ErrorAction Stop
